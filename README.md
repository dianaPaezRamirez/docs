# 📚 Documentación General - Ecosistema de Proyectos

Este repositorio centraliza la documentación técnica y operativa del ecosistema de aplicaciones y servicios.

El objetivo es mantener una fuente única de verdad para:

- onboarding de nuevos integrantes
- estándares de trabajo (Git, PRs, releases)
- arquitectura y flujos del sistema
- ambientes, servidores y bases de datos
- despliegues y operación

---

## 🧩 Proyectos activos cubiertos

Este ecosistema está compuesto por:

### 🌐 Frontend Web

- Aplicación Angular
- Aplicación React

### 📱 Mobile

- Aplicación React Native

### 🔌 Backend / APIs

- Conjunto de APIs / microservicios (backend)

---

## 📌 Índice de documentación

1. [Onboarding](./Onboarding/README.md)
2. [Git y Versionamiento](./Git/README.md)
3. [Ambientes y Servidores](./Ambiente-Servidores/README.md)
4. [Bases de Datos](./Bases-de-datos/README.md)
5. [Forma de trabajar](./07-forma-de-trabajar/README.md)

---

## 📌 Convenciones de documentación

- La documentación se escribe en formato **Markdown (.md)**.
- Toda actualización debe realizarse mediante Pull Request.
- Se recomienda mantener los documentos actualizados cuando se realicen cambios en:
  - puertos
  - variables de entorno
  - flujos de despliegue
  - arquitectura
  - reglas de Git

---

## 🤝 Cómo contribuir

1. Crear una rama (`feature/docs-...`)
2. Realizar cambios en los archivos correspondientes
3. Abrir Pull Request para revisión
4. Una vez aprobado, merge a main

---

## 📍 Nota

Este repositorio es documentación técnica.  
Documentación administrativa o de negocio (presentaciones, actas, etc.) puede mantenerse en Drive.
